...
MJExplorer uses environment-specific configuration. The configuration approach differs between local development and CI/CD deployments:

### Local Development

For local development, configure `src/environments/environment.development.ts`:

```typescript
export const environment = {
  // Local API endpoints (MJAPI running locally)
  GRAPHQL_URI: 'http://localhost:4000/',
  GRAPHQL_WS_URI: 'ws://localhost:4000/',
  REDIRECT_URI: 'http://localhost:4200/',

  // Authentication - CONFIGURE THESE VALUES
...
### CI/CD Deployments (MJ Central)

For deployed environments, the `deploy-explorer.yml` workflow **dynamically generates** `environment.ts` at build time:

1. **API URLs** are injected from GitHub variables provisioned by MJ Central:
   - `API_URL_{ENV}` → `GRAPHQL_URI`
   - Derived WebSocket URL → `GRAPHQL_WS_URI`
   - `EXPLORER_URL_{ENV}` → `REDIRECT_URI`

2. **Auth configuration** is read from the base `environment.ts` file in your repository

This means:
- You **don't need** separate environment files for each deployed environment
- API URLs are automatically correct for each environment
- Auth config (CLIENT_ID, TENANT_ID) should be configured in `environment.ts` and committed
...
**Note:** In CI/CD, the workflow always builds with `--configuration production` after injecting environment-specific values into `environment.ts`.
